...
class File::Watcher::Impl
{
public:
    Impl (File::Watcher& o, juce::File f) : owner (o), folder (f)
    {
        ...
        dispatch_queue_t queue = dispatch_queue_create("com.jreng.filewatcher", DISPATCH_QUEUE_SERIAL);
                          ...
                          const FSEventStreamEventFlags* eventFlags, const FSEventStreamEventId* eventIds)
    {
        ...
        auto safeOwner = juce::WeakReference<File::Watcher> (&impl->owner);
        ...
        for (int i = 0; i < int (numEvents); i++)
        {
            ...
            auto event = File::Watcher::Event::undefined;

            if (evt & kFSEventStreamEventFlagItemModified)
                event = File::Watcher::Event::fileUpdated;
            else if (evt & kFSEventStreamEventFlagItemRemoved)
                event = File::Watcher::Event::fileDeleted;
            else if (evt & kFSEventStreamEventFlagItemRenamed)
                event = path.exists() ? File::Watcher::Event::fileRenamedNewName : File::Watcher::Event::fileRenamedOldName;
            else if (evt & kFSEventStreamEventFlagItemCreated)
                event = File::Watcher::Event::fileCreated;

            if (event != File::Watcher::Event::undefined)
            {
...
class File::Watcher::Impl final : public juce::Thread,
...
public:
    ...
    {
        Event () = default;
        Event (const juce::File& f, const File::Watcher::Event e) : file(f), fsEvent(e) {}
        Event (const Event& other) = default;
        Event (Event&& other) = default;

        juce::File file;
        File::Watcher::Event fsEvent = File::Watcher::Event::undefined;
    ...
    Impl (File::Watcher& o, juce::File f)
      : juce::Thread ("File::Watcher::Impl"), owner (o), folder (std::move(f))
    {
        fd = inotify_init();
            ...
            while (! threadShouldExit())
            {
                ...
                if (pfd.revents & POLLIN)
                {
                    ...
                    for (const char* ptr = buf; ptr < buf + numRead; ptr += sizeof(struct inotify_event) + iNotifyEvent->len)
                    {
                        ...
                        File::Watcher::Event eventType = File::Watcher::Event::undefined;
                        if (iNotifyEvent->mask & IN_CREATE)           eventType = File::Watcher::Event::fileCreated;
                        else if (iNotifyEvent->mask & IN_CLOSE_WRITE) eventType = File::Watcher::Event::fileUpdated;
                        else if (iNotifyEvent->mask & IN_MODIFY)      eventType = File::Watcher::Event::fileUpdated;
                        else if (iNotifyEvent->mask & IN_MOVED_FROM)  eventType = File::Watcher::Event::fileRenamedOldName;
                        else if (iNotifyEvent->mask & IN_MOVED_TO)    eventType = File::Watcher::Event::fileRenamedNewName;
                        else if (iNotifyEvent->mask & IN_DELETE)      eventType = File::Watcher::Event::fileDeleted;

                        if (eventType == File::Watcher::Event::undefined)
                            continue;
...
class File::Watcher::Impl : private juce::Thread
{
public:
    Impl (File::Watcher& o, juce::File f)
      : Thread ("File::Watcher::Impl"), owner (o), folder (std::move(f))
    {
        ...
        while (! threadShouldExit())
        {
            ...
            if (success && bytesOut > 0)
            {
                ...
                while (true)
                {
                    ...
                    auto eventType = File::Watcher::Event::undefined;
                    switch (fni->Action)
                    {
                        case FILE_ACTION_ADDED:
                            eventType = File::Watcher::Event::fileCreated;
                            break;
                        case FILE_ACTION_RENAMED_NEW_NAME:
                            eventType = File::Watcher::Event::fileRenamedNewName;
                            break;
                        case FILE_ACTION_MODIFIED:
                            eventType = File::Watcher::Event::fileUpdated;
                            break;
                        case FILE_ACTION_REMOVED:
                            eventType = File::Watcher::Event::fileDeleted;
                            break;
                        case FILE_ACTION_RENAMED_OLD_NAME:
                            eventType = File::Watcher::Event::fileRenamedOldName;
                            break;
                        default:
                            break;
                    }

                    auto safeOwner = juce::WeakReference<File::Watcher> (&owner);
...
void File::Watcher::coalesceEvents (int windowMS)
{
    coalesceWindowMS = windowMS;
}

void File::Watcher::addFolder (const juce::File& folder)
{
...
void File::Watcher::removeFolder (const juce::File& folder)
{
...
void File::Watcher::removeAllFolders()
{
    watched.clear();
}

void File::Watcher::addListener (Listener* newListener)
{
    listeners.add (newListener);
}

void File::Watcher::removeListener (Listener* listener)
{
    listeners.remove (listener);
}

void File::Watcher::folderChanged (const juce::File& folder)
{
    if (coalesceWindowMS > 0)
    {
        if (timers.find (folder) == timers.end())
        {
    ...
    {
        listeners.call (&File::Watcher::Listener::folderChanged, folder);
    }
}

void File::Watcher::fileChanged (const juce::File& file, Event fsEvent)
{
    if (coalesceWindowMS > 0)
    {
    ...
    {
        listeners.call (&File::Watcher::Listener::fileChanged, file, fsEvent);
    }
}

juce::Array<juce::File> File::Watcher::getWatchedFolders()
{
